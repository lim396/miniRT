...
void	set_sphere(char **split_line, t_shape **shape_list)
{
	...
	{
		if (i == 1)
			shape_node->sphere.center = set_coordinates(split_line[i]);
		else if (i == 2)
			shape_node->sphere.radius = set_size(split_line[i]) / 2.0;
		else if (i == 3)
			shape_node->material.diffuse_ref = set_rgb(split_line[i]);
		else
			printf("ERROR\n");
		i++;
	}
...
{	
	...
	{
		if (i == 1)
			shape_node->cylinder.pos = set_coordinates(split_line[i]);
		else if (i == 2)
			shape_node->cylinder.normal= set_orientation(split_line[i]);
		else if (i == 3)
			shape_node->cylinder.radius= set_size(split_line[i]) / 2.0;
		else if (i == 4)
			shape_node->cylinder.height = set_size(split_line[i]);
		else if (i == 5)
			shape_node->material.diffuse_ref = set_rgb(split_line[i]);
		else
			printf("ERROR\n");
...
{	
	...
	{
		if (i == 1)
			shape_node->plane.pos = set_coordinates(split_line[i]);
		else if (i == 2)
			shape_node->plane.normal = set_orientation(split_line[i]);
		else if (i == 3)
			shape_node->material.diffuse_ref = set_rgb(split_line[i]);
		else
			printf("ERROR\n");
